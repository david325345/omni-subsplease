console.log(">>> SPAUŠTĚNÍ WEB UI V47 (MKV FALLBACK) <<<");
...
// --- KONFIGURACE ---
const ADDON_NAME = "SubsPlease RD v47";
...
// --- MANIFEST OBJEKT ---
const manifestObj = {
    id: 'community.subsplease.rd.v47',
    version: '37.0.0',
    name: ADDON_NAME,
    description: 'SubsPlease Addon - MKV Fallback',
...
// --- REAL-DEBRID API (MKV FALLBACK) ---
async function getRdStreamLink(magnetLink, rdToken) {
    let finalUrl = null;
    let idToResolve = null; // Deklarace pro jistotu
    ...
    try {
                ...
                if (typeof linkItem === 'string' && linkItem.startsWith('http')) {
                    ...
                    if (linkItem.includes('/d/')) {
                        ...
                        try {
                            ...
                            // PŘIHOĎ: Pokud MKV nemá stream, zkusíme link (download)
        ...
        if (!finalUrl) {
            throw new Error("RD: Nepodařilo se získat platný stream URL (zkoušeny všechny metody).");
        }
...
const streamHandler = async (id, extra) => {
    ...
    // ZDE SE ZAVOLÁ GETRDSTREAMLINK (V47)
    try {
